...
test('Check for [RESTRICTION RENDER] console messages with ALL logs captured', async ({ page }) => {
  const consoleLogs: string[] = [];
  const restrictionLogs: string[] = [];
  const xmlElementNodeStartLogs: string[] = [];
  ...
  page.on('console', msg => {
    ...
    if (text.includes('[RESTRICTION RENDER]')) {
      ...
      console.log(`✓ Found RESTRICTION RENDER: ${logEntry}`);
    }
    
    // Check for XmlElementNode START logs
    if (text.includes('[XmlElementNode START]')) {
      xmlElementNodeStartLogs.push(logEntry);
    }
  });
  ...
  const results = {
    ...
    xml_element_node_start_logs_found: xmlElementNodeStartLogs.length,
    all_logs: consoleLogs, // ALL logs
    first_20_logs: consoleLogs.slice(0, 20),
    restriction_logs: restrictionLogs,
    xml_element_node_start_logs: xmlElementNodeStartLogs,
  };

  fs.writeFileSync('./restriction-render-results-all-logs.json', JSON.stringify(results, null, 2), 'utf-8');
  fs.writeFileSync('./all-console-logs.txt', consoleLogs.join('\n'), 'utf-8');

  console.log('\n===== RESULTS =====');
  console.log(`Total console logs: ${consoleLogs.length}`);
  console.log(`[RESTRICTION RENDER] logs found: ${restrictionLogs.length}`);
  console.log(`[XmlElementNode START] logs found: ${xmlElementNodeStartLogs.length}`);
  console.log('===== FIRST 20 LOGS =====');
  consoleLogs.slice(0, 20).forEach((log, i) => console.log(`${i + 1}. ${log}`));
  console.log('===== RESTRICTION RENDER LOGS =====');
  restrictionLogs.forEach(log => console.log(log));
  console.log('===== XmlElementNode START LOGS =====');
  xmlElementNodeStartLogs.forEach(log => console.log(log));
  console.log('===================\n');
