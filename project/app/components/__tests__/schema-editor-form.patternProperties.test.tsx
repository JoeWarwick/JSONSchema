...
describe('SchemaEditorForm - patternProperties editor', () => {
  it('validates pattern key and renames pattern property on valid input', async () => {
    ...
    render(
      <TooltipProvider>
        <SchemaEditorForm schema={testSchema} onChange={handleChange} />
      </TooltipProvider>
    );
    ...
    expect(Object.keys(latest.patternProperties)).toContain('^job_');
    ...
    await waitFor(() => {
      expect(latest.patternProperties).toBeDefined();
      expect(Object.keys(latest.patternProperties)).toContain('^jobX_');
      expect(Object.keys(latest.patternProperties)).not.toContain('^job_');
    });
  });
});